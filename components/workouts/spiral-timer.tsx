...
export default function SpiralTimer() {
  ...
  useEffect(() => {
    ...
    intervalRef.current = requestAnimationFrame(tick)
    return () => {
      if (intervalRef.current != null) cancelAnimationFrame(intervalRef.current)
    }
  ...
  const sanitize = (val: string, max: number) => {
  ...
  return (
    <div className="relative w-full h-full min-h-[560px] flex items-center justify-center bg-[#05070a] p-6 font-mono select-none overflow-hidden">
      {/* Backdrop grid + scanlines — scoped to this container, not the viewport */}
      <div
      ...
      <div className="relative w-full max-w-md flex flex-col items-center gap-8">
        ...
        <div className="relative">
          ...
          >
            ...
            <circle cx="150" cy="150" r="140" fill="none" stroke="rgba(16, 185, 129, 0.15)" strokeWidth="1" />
            <circle cx="150" cy="150" r="132" fill="none" stroke="rgba(16, 185, 129, 0.35)" strokeWidth="1" strokeDasharray="2 6" />
            <circle cx="150" cy="150" r="122" fill="none" stroke="rgba(16, 185, 129, 0.1)" strokeWidth="1" />

            <path d={spiralPath} fill="none" stroke="rgba(16, 185, 129, 0.08)" strokeWidth="3" strokeLinecap="round" />
            ...
            {isRunning && (
              <g style={{ transformOrigin: '150px 150px', animation: 'spiral-timer-spin 8s linear infinite' }}>
            ...
            {isEditing ? (
              ...
              <motion.button
                ...
                className={`text-6xl sm:text-7xl font-mono tracking-tight tabular-nums transition ${
                  isFinished ? 'text-emerald-100' : 'text-emerald-200'
      ...
      <style
        dangerouslySetInnerHTML={{
          __html:
            '@keyframes spiral-timer-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }',
        }}
      />
    </div>
  )
