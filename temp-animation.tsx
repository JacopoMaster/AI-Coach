...
/* ═══════════════════════════════════════════════════════════
   ...
   La trivella SVG ora ha:
   - viewBox "-100 0 200 320"
   - punta a (0, 0) ← in cima esatta del viewBox
   - base a y=300
   
   Quindi se metto il wrapper della trivella con `top: 50%`,
   la punta sta esattamente a 50% dello schermo. Niente più
   calcoli derivati: 1:1.
   ═══════════════════════════════════════════════════════════ */
const BLOCK = {
  widthPct: 70,
  heightPct: 35,
  bottomYPct: 50,  // bordo inferiore del blocco a metà schermo
};
BLOCK.topYPct = BLOCK.bottomYPct - BLOCK.heightPct;       // 15
BLOCK.leftXPct = (100 - BLOCK.widthPct) / 2;              // 15
BLOCK.rightXPct = BLOCK.leftXPct + BLOCK.widthPct;        // 85

// Wrapper trivella: altezza in % dello schermo
const DRILL_HEIGHT_PCT = 50;
const DRILL_WIDTH_PCT = 30;

// Posizioni del wrapper (top in %):
// - DOCKED: punta esattamente a y=BLOCK.bottomYPct
const DRILL_DOCKED_TOP_PCT = BLOCK.bottomYPct;          // 50
const DRILL_START_TOP_PCT = 110;                         // sotto lo schermo
const DRILL_EXIT_TOP_PCT = -DRILL_HEIGHT_PCT - 10;       // sopra lo schermo
...
export default function App() {
  ...
  const [msg, setMsg] = useState("LEVEL UP");

  const trigger = (newLevel, message) => {
    setLevel(newLevel);
    setMsg(message);
    setActive(true);
  };
  ...
  return (
    <div style={{
      ...
      <h1 style={{
        fontSize: "20px", margin: 0, color: "#00f0ff",
        textShadow: "0 0 20px #00f0ff", letterSpacing: "0.1em", fontWeight: 300
      }}>
        GIGA DRILL · MOBILE
      </h1>
      ...
      <div style={{
        ...
        boxShadow: "0 20px 60px rgba(0, 240, 255, 0.15), inset 0 0 0 1px #2a2a3a",
        overflow: "hidden",
      }}>
        <div style={{
          position: "absolute", top: "12px", left: "50%", transform: "translateX(-50%)",
          width: "100px", height: "22px", background: "#000", borderRadius: "12px", zIndex: 100,
        }} />

        <div style={{
          position: "absolute", inset: 0, padding: "60px 20px 20px",
          display: "flex", flexDirection: "column", gap: "12px",
          opacity: active ? 0.3 : 1, transition: "opacity 0.4s",
        }}>
          <div style={{ fontSize: "11px", color: "#00f0ff", letterSpacing: "0.3em", opacity: 0.6 }}>▸ DASHBOARD</div>
          <div style={{ fontSize: "32px", fontWeight: 200, color: "#fff", letterSpacing: "0.05em" }}>
            LEVEL {level}
          </div>
          <div style={{ height: "1px", background: "linear-gradient(90deg, #00f0ff, transparent)", marginBottom: "8px" }} />
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            <Stat label="STREAK" value="14d" color="#ff00ff" />
            <Stat label="SPIRAL POWER" value={`${level * 137}`} color="#00f0ff" />
            <Stat label="WORKOUTS" value="86" color="#ffcc40" />
          </div>
        </div>
      ...
      <div style={{
        ...
        {TIERS.map((t) => {
          const c = t.aura?.color || "#888";
          return (
            <button
              key={t.id}
              onClick={() => trigger(t.minLevel, t.label)}
              style={{
...
function Stat({ label, value, color }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: "10px", color: "#888", letterSpacing: "0.2em" }}>{label}</span>
      <span style={{ fontSize: "13px", color, fontFamily: "monospace" }}>{value}</span>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════
   GIGA DRILL BREAK
   ═══════════════════════════════════════════════════════════ */
function GigaDrillBreak({ show = false, onComplete = () => {}, message = "BREAKTHROUGH", level = 1 }) {
  ...
  useEffect(() => {
    ...
    const timers = [
      ...
      setTimeout(() => { setPhase("done"); onComplete(); }, 7800),
    ];
  ...
  const accentColor = tier.aura?.color || "#00f0ff";
    ...
    <AnimatePresence>
      {phase !== "done" && (
        <motion.div
          key="overlay"
          style={{
            position: "absolute", inset: 0, zIndex: 999, overflow: "hidden",
            background: tier.megaForm ? "rgba(5, 0, 25, 0.94)"
                      : tier.cosmic ? "rgba(8, 0, 20, 0.93)"
                      : "rgba(5, 5, 15, 0.92)",
          ...
          animate={{
            opacity: 1,
            x: screenShake ? [0, -8 * shakeIntensity, 10 * shakeIntensity, -6 * shakeIntensity, 8 * shakeIntensity, -3 * shakeIntensity, 0] : 0,
            y: screenShake ? [0, 6 * shakeIntensity, -8 * shakeIntensity, 4 * shakeIntensity, -6 * shakeIntensity, 2 * shakeIntensity, 0] : 0,
          }}
          exit={{ opacity: 0 }}
          transition={{
        ...
        >
          ...
          {tier.crossStars > 0 && (
            <BackgroundStars count={tier.crossStars} color={tier.megaForm ? "#fff080" : accentColor2} />
          )}
          ...
          <AnimatePresence>
            {phase === "drilling" && (
              <Sparks key="sparks" color={tier.sparkColor} />
            )}
          </AnimatePresence>
          ...
          <AnimatePresence>
            {(phase === "reform" || phase === "text") && (
              <ReformedBlock key="block-reformed" phase={phase} message={message} tier={tier} />
            )}
          </AnimatePresence>

          <AnimatePresence>
            {(phase === "rise" || phase === "drilling" || phase === "shatter" || phase === "pierce") && (
...
/* ═══════════════════════════════════════════════════════════
   ...
   - wrapper top = posizione della punta (0% del wrapper = top)
   - perché nel SVG la punta è a (0,0) del viewBox
   ═══════════════════════════════════════════════════════════ */
function DrillProjectile({ phase, tier }) {
  const targetTop = phase === "rise" || phase === "drilling" || phase === "shatter"
                    ? `${DRILL_DOCKED_TOP_PCT}%`
                  : phase === "pierce"
                    ? `${DRILL_EXIT_TOP_PCT}%`
                    : `${DRILL_START_TOP_PCT}%`;

  const duration = phase === "rise" ? 0.7
                 : phase === "drilling" ? 0
                 : phase === "shatter" ? 0
                 : phase === "pierce" ? 0.7
                 : 0;
  ...
  const baseShakeClass = tier.megaForm ? "drill-shake-mega"
                       : tier.id >= 9 ? "drill-shake-chromatic" : "";
  ...
  return (
    <motion.div
      ...
      transition={{
        duration,
        ease: phase === "rise" ? [0.4, 0, 0.3, 1]
            : phase === "pierce" ? [0.7, 0, 0.4, 1]
            : "linear",
      }}
    >
      <style>{`
...
/* ═══════════════════════════════════════════════════════════
   SVG TRIVELLA — RISCRITTA con punta a (0, 0)
   
   viewBox: -100 0 200 320
   - x: da -100 a +100 (centrato sull'asse Y)
   - y: da 0 a 320
   - PUNTA: (0, 0) ← in cima esatta
   - BASE: y=300 (ellissi a y=300 con rim a y=300, body a y=298)
   
   Gli effetti laterali (aura, rays, shock, lightning) sono
   centrati su (0, 160) — il centro del corpo della trivella.
   ═══════════════════════════════════════════════════════════ */
function SpiralDrillSVG({ tier }) {
  const TIP_X = 0;
  const TIP_Y = 0;
  const BASE_Y = 280;
  const BASE_HALF_WIDTH = 78;
  const CY_FX = 140;  // centro corpo (per effetti)
      ...
      >
        <defs>
          {[
            ...
            <linearGradient key={name} id={`bodyGrad_${name}_${tier.id}`} x1="0" y1="0" x2="1" y2="0">
          ...
          <clipPath id={`drillCone_${tier.id}`}>
            <path d={`M ${TIP_X} ${TIP_Y} L ${TIP_X + BASE_HALF_WIDTH} ${BASE_Y} L ${TIP_X - BASE_HALF_WIDTH} ${BASE_Y} Z`} />
          </clipPath>
        ...
        {/* Effetti laterali centrati su (0, CY_FX) */}
        {tier.aura && (
          <ellipse className={`${sid}-aura`} cx={0} cy={CY_FX}
            rx={100 * tier.aura.size * 0.7} ry={180 * tier.aura.size * 0.7}
            fill={`url(#auraGrad_${tier.id})`} opacity={tier.aura.opacity} />
        )}
        {tier.rays > 0 && (
          <KineticRays count={tier.rays} color={tier.aura ? tier.aura.color : "#ffffff"}
            length={tier.megaForm ? 60 : 30} cx={0} cy={CY_FX} sid={sid} />
        )}
        {tier.shockRing && tier.aura && (
          <circle cx={0} cy={CY_FX} r="100" fill="none"
            stroke={tier.aura.color} strokeWidth="2" opacity="0.6" className={`${sid}-shock`} />
        )}
        {tier.lightning && tier.aura && <Lightning color={tier.aura.color} sid={sid} cx={0} cyTop={TIP_Y} />}
        {tier.particles > 0 && <Particles count={tier.particles} color={tier.grooveHi} cx={0} cy={CY_FX} sid={sid} />}
        ...
        <path d={`M ${TIP_X} ${TIP_Y} L ${TIP_X + BASE_HALF_WIDTH} ${BASE_Y} L ${TIP_X - BASE_HALF_WIDTH} ${BASE_Y} Z`}
          fill={bodyFill} stroke={tier.rim} strokeWidth="1.5" />
        ...
        <g clipPath={`url(#drillCone_${tier.id})`}>
          {strands.map((d, i) => (
            <g key={i}>
              ...
              <path d={d} fill="none" stroke={tier.grooveMid} strokeWidth="5" strokeLinecap="round" strokeDasharray="30 6" className={`${sid}-strand-main`} />
              <path d={d} fill="none" stroke={tier.grooveHi} strokeWidth="2" strokeLinecap="round" strokeDasharray="20 16" className={`${sid}-strand-hl`} opacity="0.9" />
            </g>
          ))}
          <rect x={-45} y="0" width="35" height="320" fill={`url(#shineGrad_${tier.id})`} className={`${sid}-shine`} style={{ mixBlendMode: "screen" }} />
        </g>

        <path d={`M ${TIP_X} ${TIP_Y} L ${TIP_X + BASE_HALF_WIDTH} ${BASE_Y} L ${TIP_X + 30} ${BASE_Y} Z`}
          fill="black" opacity={tier.id >= 9 ? 0.1 : 0.3} pointerEvents="none" />

        {tier.tipGlow && (
          <>
            <g style={{ transformOrigin: `0px ${TIP_Y}px` }} className={`${sid}-tip-star`}>
              <CrossStar cx={0} cy={TIP_Y}
                size={tier.megaForm ? 22 : tier.id >= 10 ? 18 : tier.id >= 8 ? 14 : tier.id >= 7 ? 10 : tier.id >= 3 ? 6 : 4}
                color="#ffffff" />
            </g>
            {tier.id >= 7 && (
        ...
        <path d={`M ${TIP_X} ${TIP_Y} L ${TIP_X - BASE_HALF_WIDTH} ${BASE_Y}`}
          fill="none" stroke={tier.grooveHi} strokeWidth="1.2" opacity="0.6" />
      </svg>
    </>
  );
}
...
/* ═══════════════════════════════════════════════════════════
   Effetti vari
   ═══════════════════════════════════════════════════════════ */
function CrossStar({ cx, cy, size, color }) {
  const s = size;
  return (
    <g transform={`translate(${cx} ${cy})`} style={{ transformOrigin: `${cx}px ${cy}px` }}>
      <path d={`M 0 ${-s} L ${s * 0.12} ${-s * 0.12} L ${s} 0 L ${s * 0.12} ${s * 0.12} L 0 ${s} L ${-s * 0.12} ${s * 0.12} L ${-s} 0 L ${-s * 0.12} ${-s * 0.12} Z`} fill={color} />
      <path d={`M 0 ${-s * 0.5} L ${s * 0.05} ${-s * 0.05} L ${s * 0.5} 0 L ${s * 0.05} ${s * 0.05} L 0 ${s * 0.5} L ${-s * 0.05} ${s * 0.05} L ${-s * 0.5} 0 L ${-s * 0.05} ${-s * 0.05} Z`} fill="white" opacity="0.9" />
    </g>
  );
}

function Particles({ count, color, cx, cy, sid }) {
  return (
    <g>
      {Array.from({ length: count }).map((_, i) => {
        ...
        const size = 1 + seed * 2;
        return (
          <circle key={i} cx={x} cy={y} r={size} fill={color}
...
function KineticRays({ count, color, cx, cy, length, sid }) {
  return (
    <g className={`${sid}-kinetic`} style={{ transformOrigin: `${cx}px ${cy}px` }}>
      {Array.from({ length: count }).map((_, i) => {
        const angle = (i / count) * 360;
        return (
          <line key={i} x1={cx} y1={cy - 130} x2={cx} y2={cy - 130 - length}
            stroke={color} strokeWidth="3" strokeLinecap="round" opacity="0.8"
            transform={`rotate(${angle} ${cx} ${cy})`}
            style={{ animation: `${sid}-ray-pulse 1.2s ease-in-out ${(i / count) * 0.8}s infinite` }} />
        );
      })}
    </g>
  );
...
function Lightning({ color, sid, cx, cyTop }) {
  const bolts = [
  ...
  return (
    <g className={`${sid}-lightning`}>
      {bolts.map((d, i) => (
        <path key={i} d={d} fill="none" stroke={color} strokeWidth={2 + (i % 2)}
...
function BackgroundStars({ count, color }) {
  const stars = useMemo(() => {
    ...
    for (let i = 0; i < Math.min(count, positions.length); i++) {
      arr.push({ ...positions[i], size: 6 + (i % 4) * 2, delay: (i * 0.2) % 1.6, duration: 1.6 + (i % 3) * 0.4 });
    }
    return arr;
  }, [count]);
          ...
          return (
            <g key={i} transform={`translate(${s.x} ${s.y})`}
              style={{
                ...
                transformOrigin: `${s.x}px ${s.y}px`, transformBox: "fill-box",
              }}>
              <path d={`M 0 ${-sz} L ${sz * 0.12} ${-sz * 0.12} L ${sz} 0 L ${sz * 0.12} ${sz * 0.12} L 0 ${sz} L ${-sz * 0.12} ${sz * 0.12} L ${-sz} 0 L ${-sz * 0.12} ${-sz * 0.12} Z`}
                fill={color} style={{ filter: `drop-shadow(0 0 2px ${color})` }} />
              <path d={`M 0 ${-sz * 0.5} L ${sz * 0.05} ${-sz * 0.05} L ${sz * 0.5} 0 L ${sz * 0.05} ${sz * 0.05} L 0 ${sz * 0.5} L ${-sz * 0.05} ${sz * 0.05} L ${-sz * 0.5} 0 L ${-sz * 0.05} ${-sz * 0.05} Z`}
...
function CosmicBackdrop({ megaForm }) {
  return (
...
/* ═══════════════════════════════════════════════════════════
   BLOCCO DI ROCCIA SOLIDA
   
   Pietra grezza opaca, niente trasparenze. Texture con:
   - colore base scuro (gradient grigio-marrone)
   - macchie scure pseudo-random distribuite a coprire l'intera area
   - bevel chiaro in alto + ombra in basso (illuminazione dall'alto)
   - bordo neon esterno per coerenza estetica
   
   Crepe in due ondate:
   - 7 crepe principali con ramificazioni: si propagano nei
     primi 1.8s, raggi fino al bordo opposto del blocco
   - 14 crepe secondarie più piccole: appaiono nell'ultima parte
     del drilling per riempire le zone non coperte
   ═══════════════════════════════════════════════════════════ */
function BlockSolid({ accentColor, accentColor2, drilling }) {
  const { mainCracks, secondaryCracks, rockSpots } = useMemo(() => {
    const main = [];
    const secondary = [];
    const spots = [];

    // Punto di contatto: centro-basso del viewBox 400x200
    const startX = 200;
    const startY = 200;

    // 7 crepe principali distribuite nell'arco superiore
    const numMain = 7;
    for (let i = 0; i < numMain; i++) {
      // Angoli da -160° a -20° (verso l'alto, ben distribuiti)
      const angleDeg = -160 + (i / (numMain - 1)) * 140;
      const angle = (angleDeg * Math.PI) / 180;

      // Lunghezza tale da raggiungere il bordo opposto del blocco
      const len = 280;
      const segments = 6;
    ...
    // 14 crepe secondarie sparse nel blocco
    ...
    // Macchie scure sulla pietra (distribuzione pseudo-random deterministica)
    for (let i = 0; i < 35; i++) {
      const sx = (i * 73) % 400;
      const sy = (i * 53) % 200;
  ...
  return (
    ...
    >
      <svg viewBox="0 0 400 200" preserveAspectRatio="none" style={{ width: "100%", height: "100%" }}>
        <defs>
          {/* Pietra solida: gradient grigio-marrone con luce dall'alto */}
        ...
        {drilling && (
          <g>
            {mainCracks.map((c, i) => (
              <g key={`m-${i}`}>
                {/* Doppio tratto: scuro sotto, chiaro sopra → effetto incisione 3D */}
                <motion.path
                  d={c.main} fill="none" stroke="#000" strokeWidth="3.5" strokeLinecap="round"
                  initial={{ pathLength: 0, opacity: 0 }}
                  animate={{ pathLength: 1, opacity: 0.85 }}
                  transition={{ duration: c.duration, delay: c.delay, ease: "easeOut" }}
                />
                <motion.path
                  d={c.main} fill="none" stroke="#ffffff" strokeWidth="1.5" strokeLinecap="round"
                  initial={{ pathLength: 0, opacity: 0 }}
                ...
                {c.branches.map((b, j) => (
                  <g key={`b-${j}`}>
                    <motion.path
                      d={b} fill="none" stroke="#000" strokeWidth="2.2" strokeLinecap="round" opacity="0.6"
                      initial={{ pathLength: 0, opacity: 0 }}
                      animate={{ pathLength: 1, opacity: 0.7 }}
                      transition={{ duration: c.duration * 0.7, delay: c.delay + 0.15, ease: "easeOut" }}
                    />
                    <motion.path
                      d={b} fill="none" stroke="#ffffff" strokeWidth="1" strokeLinecap="round" opacity="0.8"
                      initial={{ pathLength: 0, opacity: 0 }}
            ...
            {secondaryCracks.map((c, i) => (
              <g key={`s-${i}`}>
                ...
                />
                <motion.path
                  d={c.main} fill="none" stroke="#ffffff"
                  strokeWidth="0.8" strokeLinecap="round" opacity="0.7"
                  initial={{ pathLength: 0, opacity: 0 }}
            ...
            {/* Pulse ripetuto al punto d'impatto (ribadisce "sta scavando") */}
            <motion.circle
              cx="200" cy="200" r="6" fill="#ffffff"
              animate={{
                scale: [0, 3, 0],
                opacity: [1, 0, 0]
              }}
...
/* ═══════════════════════════════════════════════════════════
   FRAMMENTI DEL BLOCCO (di pietra)
   ═══════════════════════════════════════════════════════════ */
function BlockShards({ phase, accentColor, accentColor2 }) {
    ...
    <div style={{
      ...
      >
        ...
        {shards.map((s) => {
          ...
          let animate, duration, ease;

          if (phase === "shatter") {
            animate = {
              x: explodeX, y: explodeY,
              rotate: s.rotationDelta,
              rotateX: s.tumbleX * 0.5, rotateY: s.tumbleY * 0.5,
              opacity: 1, scale: 1,
            };
            duration = 0.5;
            ...
            animate = {
              x: explodeX * 1.2, y: explodeY + 80,
              rotate: s.rotationDelta + s.fallSpin * 0.3,
              rotateX: s.tumbleX * 0.7, rotateY: s.tumbleY * 0.7,
            ...
            animate = {
              ...
              rotateX: s.tumbleX, rotateY: s.tumbleY,
              opacity: 0, scale: 0.7,
            };
            duration = 1.2;
            ease = [0.4, 0, 0.7, 1];
          ...
          return (
            ...
            >
              ...
              <polyline
                ...
                strokeWidth="0.6" strokeLinecap="round" opacity="0.7"
...
function generateBlockShards() {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const seed = id;
      const jx = (n) => ((seed * (n + 7)) % 13) - 6;
      const jy = (n) => ((seed * (n + 11)) % 13) - 6;
...
function buildBlockShard(id, pointsArray, flyAngle, flyDistance, seed) {
  let sx = 0, sy = 0;
  for (const p of pointsArray) { sx += p[0]; sy += p[1]; }
  ...
  const highlightPoints = pointsArray.slice(0, 3).map(p => `${p[0].toFixed(1)},${p[1].toFixed(1)}`).join(" ");
...
/* ═══════════════════════════════════════════════════════════
   BLOCCO RICOMPOSTO
   ═══════════════════════════════════════════════════════════ */
function ReformedBlock({ phase, message, tier }) {
  const accentColor = tier.aura?.color || "#00f0ff";
  const accentColor2 = tier.grooveHi || "#ff00ff";
  ...
  return (
    ...
    >
      <svg viewBox="0 0 400 200" preserveAspectRatio="none"
        ...
        <rect x="3" y="3" width="394" height="194"
          fill="url(#reformedBlockFill)" stroke={accentColor} strokeWidth="2.5"
          style={{ filter: `drop-shadow(0 0 14px ${accentColor})` }} />

        <motion.line x1="3" x2="397" y1="40" y2="40"
          stroke={accentColor2} strokeWidth="1" opacity="0.6"
      ...
      <AnimatePresence>
        {phase === "text" && (
          ...
          >
            <div style={{
              fontSize: "9px", color: accentColor2,
